...
AUTOSTART_PROCESSES(
    &beacon_process,
    &button_process); /* Start both processes when the node boots */
/*---------------------------------------------------------------------------*/
...
PROCESS_THREAD(beacon_process, ev, data) {
  ...
  while (1) {
    ...
    if (ev == PROCESS_EVENT_TIMER && etimer_expired(&et)) {
      packetbuf_clear();
      broadcast_send(
          &bc_conn); /* We just send an empty packet in broadcast... Why?!
                      * NB: We rely on the identified sender broadcast primitive
                      * of RIME. Therefore, RIME will add the sender address as
                      * an attribute to our payload.
                      */
    }
  }
  PROCESS_END();
...
static void send_msg() {
  ...
  static uint16_t seqn =
      0; /* Increase this number by 1 upon every button press */
  uint8_t hops = 0;
  ...
  memcpy(payload + idx, str, str_length);
  packetbuf_set_datalen(idx + str_length);
...
static void recv_uc(struct unicast_conn *c, const linkaddr_t *from) {
  ...
  uint16_t idx = 0;
  seqn = (uint16_t)payload[idx++] << 8;
  seqn |= (uint16_t)payload[idx++];

  hops = payload[idx++];

  size_t str_len = packetbuf_datalen() - idx;
  char str[str_len + 1];
  memcpy(str, payload + idx, str_len);
  str[str_len] = '\x00';

  printf("Recv from %02x:%02x length=%u seqn=%u hopcount=%u\n", from->u8[0],
         from->u8[1], packetbuf_datalen(), seqn, hops);
  printf("%s\n", str);
...
/*---------------------------------------------------------------------------*/
linkaddr_t nbr_table[NBR_TABLE_SIZE]; // Simplistic neighbor table (just an
                                      // array of addresses)
...
static void nbr_table_add(const linkaddr_t *addr) {
  ...
  nbr_table[cur_nbr_idx] =
      *addr; // Store the new neighbor, potentially in place of an old one
